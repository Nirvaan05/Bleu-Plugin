...
3. **Agent team** — Four locked-tool subagents that hand off through files **with hook-driven transitions**: **Researcher** (web → `raw/research/`), **Curator** (`raw/` → `plan/`, rebuilds index and graph, has `memory: project` for institutional knowledge), **Linter** (read-only, writes to `.reflection/proposals/`), **Auditor** (implemented as an `agent` hook on `SubagentStop` matched to the linter — fires automatically, no separate file). Proposer-validator separation is enforced: same agent never both proposes and approves a change.
...
7. **External integrations** — MCP servers that ingest GitHub PRs/issues, Linear/Jira tickets, meeting transcripts, and web search results into `raw/` automatically. The Curator picks them up via the same `PostToolUse` hook used elsewhere.
